...
Editor::Editor(Processor& p) : AudioProcessorEditor(p),processor(p),skin(std::make_unique<Skin>()),canvas(std::make_unique<Canvas>(*this)),exporter(std::make_unique<ExportHandle>(*this)) {
    ...
    play.onClick=[this]{if(processor.engine.uiMeter.load() && processor.engine.uiTempoKnown.load())processor.session.play();};stop.onClick=[this]{processor.session.stop();};start.onClick=[this]{processor.session.seek(0);};sync.onClick=[this]{processor.session.setSync(!processor.session.sync);changed();};
...
void Editor::handleAsyncUpdate() {
    ...
    repeats.setToggleState(s.repeats,juce::dontSendNotification);sync.setToggleState(s.sync,juce::dontSendNotification);play.setEnabled(!s.sync && processor.engine.uiMeter.load() && processor.engine.uiTempoKnown.load());stop.setEnabled(!s.sync);
