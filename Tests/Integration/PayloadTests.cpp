...
int main(int argc,char** argv){
    auto build=std::filesystem::path(argc>1 ? argv[1] : ".");auto root=ownedWorkspace(build,"payload-tests-");
    ...
    write(root/receiptName,receipt+sha256(text)+"\tContents/resources/changed.txt\n");
    require(rejects([&]{payload.validate(root,false);}),"captured ownership receipt cannot change between validation and staging");write(root/receiptName,receipt);
    ...
    write(root/receiptName,receipt+sha256(text)+"\tCHORDCANVAS.PAYLOAD\n");require(rejects([&]{Payload::read(root);}),"ownership receipt cannot be listed under a Windows case alias");
